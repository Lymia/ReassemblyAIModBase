...
# - glew (glew32.dll)
# - glm (statically linked)
# - outlaws-core (statically linked)
# - zlib (zlib1.dll)
# - Zydis (not used in Reassembly)
...
for i in chipmunk core glew glm zlib zydis; do
	rm -rvf $i || exit 1
	mkdir -v $i || exit 1	
done
...
wget -O Chipmunk2D-$CHIPMUNK_VERSION.tgz https://github.com/Lymia/Chipmunk2D/archive/$CHIPMUNK_VERSION.tar.gz || exit 1
...
cp -v outlaws-core/{LICENSE,*.h,{base64,md5}.c,{base64,Geometry,Nav,Particles,stl_ext,Str,phpbbhash}.cpp} "$LIBS/core" || exit 1
...
wget -O glm-$GLM_VERSION.tar.gz https://github.com/g-truc/glm/archive/$GLM_VERSION.tar.gz || exit 1
...
# Download zydis sources
wget -O zydis-$ZYDIS_VERSION.tar.gz https://github.com/zyantific/zydis/archive/v$ZYDIS_VERSION.tar.gz || exit 1
tar xvf zydis-$ZYDIS_VERSION.tar.gz || exit 1
cp -vr zydis-$ZYDIS_VERSION/{LICENSE,include,src} "$LIBS/zydis" || exit 1
cp -v zydis-$ZYDIS_VERSION/msvc/*.h "$LIBS/zydis/include"
